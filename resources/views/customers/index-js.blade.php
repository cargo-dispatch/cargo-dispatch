...
$('#searchInput').on('keyup', function () {
    ...
    debounceTimer = setTimeout(function () {
        loadDrivers(1, parseInt($('#perPageSelect').val()) || 10, searchTerm);
    }, 300);
});
...
$('#clearFilters').on('click', function () {
    ...
    if (fpTo) fpTo.clear();
    $('#searchInput').val('');
    loadDrivers(1, parseInt($('#perPageSelect').val()) || 10, '');
});
...
function loadDrivers(page = 1, perPage = 10, searchTerm = '', showLoader = true) {
        ...
        success: function(response) {
            ...
            if (data.length === 0) {
                $('#userTableBody').append(`
                    <tr>
                        <td colspan="7" class="text-center pg-empty-cell">No records found</td>
                    </tr>
                `);
            } else {
    ...
    ${user.state ? ', ' + user.state : ''}
                            ...
                            <td style="white-space:nowrap;">
                                ...
                                <a href="${user.actions.edit}" class="text-primary me-1" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <button type="button" class="btn btn-link p-0 me-1 toggle-status-btn ${user.is_active ? 'text-success' : 'text-secondary'}"
                                    title="${user.is_active ? 'Deactivate' : 'Activate'}"
                                    data-id="${user.id}"
                                    data-active="${user.is_active}"
                                    data-url="${user.actions.toggle_status}">
                                    <i class="bi ${user.is_active ? 'bi-toggle-on' : 'bi-toggle-off'}" style="font-size:1.1rem;vertical-align:middle;"></i>
                                </button>
        ...
        error: function(error) {
            ...
            Swal.fire('Error', 'Failed to load customers data.', 'error');
        },
        complete: function() {
...
$('#roleForm').on('submit', function (e) {
    ...
    $.ajax({
        ...
        success: function (response) {
            Swal.fire('Success', 'Customer  has been saved successfully!', 'success').then(() => {
        ...
        error: function (xhr) {
            ...
            if (xhr.status === 422) {
                ...
                $.each(errors, function (key, value) {
                    ...
                    const parentDiv = input.closest('.col-md-6, .col-md-4, .col-md-3, .col-md-12');
...
document.addEventListener('DOMContentLoaded', function () {
    document.addEventListener('click', function (e) {
        if (e.target.closest('.view-customer-details')) {
            ...
            showDetailModal({
                route: `${APP_URL}/admin/customers/customers/details/${customerId}`,
    ...
    { label: 'Address', key: 'full_address' },  // Add this line
],
                renderExtras: (data) => {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $btn.data('active', active)
                    .attr('title', active ? 'Deactivate' : 'Activate')
                    .removeClass('text-success text-secondary')
                    .addClass(active ? 'text-success' : 'text-secondary')
                    .find('i')
...
$(document).on('click', '.delete-button', function () {
    ...
    const deleteUrl = "{{ route('customers.destroy', ':id') }}".replace(':id', userId);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                error: function (error) {
                    console.error('Error deleting Customers type:', error.responseJSON.message);
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function(response) {
                    Swal.fire('Deleted!', `${selectedIds.length} customer(s) have been deleted.`, 'success');
                ...
                error: function(error) {
                    ...
                    Swal.fire('Error', 'Failed to delete selected customer.', 'error');
                }
            });
        }
    });
